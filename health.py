...
temp = .4
key_file_path = os.getcwd()+'/SA.json'
...
def get_api_key(secret_id, project_id):
    ...
    # Return the secret payload as a decoded string
    api_key = response.payload.data.decode("UTF-8")
    return(api_key)
project_id = "eloquent-victor-441420-k3"
secret_id = "GOOGLE_API_KEY"
get_api_key(secret_id, project_id)
...
api_key = get_api_key(secret_id, project_id)
GOOGLE_API_KEY = api_key
...
CLOUD_STORAGE_BUCKET = 'wellness-simplified-embedded-docs'
...
filter_list = []
lang_list = ['English', 'Japanese', 'Korean', 'Arabic', 'Bahasa Indonesia', 'Bengali', 'Bulgarian', 'Chinese',
             'Croatian', 'Czech', \
             'Danish', 'Dutch', 'Estonian', 'Finnish', 'French', 'German', 'Gujarati', 'Greek', 'Hebrew', 'Hindi',
             'Hungarian', 'Italian', \
             'Kannada', 'Latvian', 'Lithuanian', 'Malayalam', 'Marathi', 'Norwegian', 'Polish', 'Portuguese',
...
with st.sidebar:
    ...
    with col2:
        st.markdown(
            "<h3 style='text-align: left'>MultiLingual Health BOT - Wellness-Simplified : Is your food healthy? and Health QA.  </h3>",
            unsafe_allow_html=True)
        st.write(
            "This application uses Gemini Model. It can tell if your food is healthy by a picture of it. Also, it can get the answers from a repository of documents.")
    col1, col2, col3 = st.columns([.42, .29, .29])
    with col1:
        appmode = st.radio("**App Mode**",
                           ['Is Your Food Healthy?', 'All Docs QA', 'Single Doc QA', 'General Health QA'], index=0,
                           help="You may get answer from all documents or single document using Apllication Mode.")
    ...
    with col2:
        ...
        model_name = st.selectbox("**LLM Name**", ["gemini-1.5-pro-002", "gemini-1.5-flash-002"], index=0,
                                  help="Select a model to get the answer.")

    with col3:
        num_chunks = st.number_input('**Number of Chunks**', min_value=1, max_value=15, value=3, step=1,
                                     help="Number of chunks to be used to answer.")
        temp = st.number_input('**Temperature**', min_value=0.0, max_value=1.0, value=0.4, step=0.1,
                               help="Temperature for Gemini Model. Increase it for more creative answers.")
    ...
    if appmode == 'All Docs QA':
        ...
        st.write(
            "You may ask any health related question. It will provide general health advice without stored documents repository.")
    ...
    st.write("For questions or feedback, please reach out to us. Email: ipsitalearn2025@gmail.com")
...
if appmode == 'All Docs QA':
    ...
    st.write(
        f"Selected document to get the answer: {doc_names}. Only this document will be used to answer the question.")
else:
    retriever = vectordb.as_retriever(search_kwargs={"k": num_chunks})
...
if appmode == 'Is Your Food Healthy?':
    ...
    if uploaded_file is not None:
        ...
        if food_desc == "Not a food item":
            ...
            health_tip = llm.invoke(
                f"Is {food_desc} healthy? Provide helath and nutrition related information about: {food_desc}").content
            if lang != 'English':
                try:
    ...
    general_question = st.text_input("**Type your general health question below and press enter.**", value="",
                                     help="Type your general health question here. Example: I have cough and cold , what should I do?")
    ...
    question = st.text_input("**Type your question below.**", value="",
                             help="Type your question here. Example: How to prevent childhood obesity?")
    col1, col2, col3, col4 = st.columns([.25, .30, .30, .15])
    with col1:
        show_sources = st.checkbox("Show Sources", value=True,
                                   help="Show source document and page numbers used to generate the answer.")
    with col2:
        print_text = st.checkbox("Show Text Chunks ", value=False,
                                 help="Print text chunks, used to answer the question.")
    with col3:
        pass
    with col4:
